...
st.write("🔍 App is starting...")  # Debugging checkpoint
...
st.write(f"✅ Secrets loaded: OPENAI={bool(OPENAI_API_KEY)}, MCP={bool(MCP_PIPEDREAM_URL)}, PINECONE={bool(PINECONE_PLUGIN_API_KEY)}")  # Debugging checkpoint
...
st.write("✅ LLM Initialized")  # Debugging checkpoint
...
try:
    ...
    st.write("✅ Pinecone Assistant initialized")  # Debugging checkpoint
except Exception as e:
    st.error(f"❌ Failed to initialize Pinecone Assistant: {e}")
    st.stop()

# Optional: Assign to global variable if needed for downstream
pinecone_assistant_initialized = True
